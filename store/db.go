...
func init() {
	...
	if err := myDB.Create("Clusters"); err != nil {
		panic(err)
	}

	store = myDB.Use("Feeds")
}

func GetClusters() ([]*cluster.Cluster, error) {
	cs := make([]*cluster.Cluster, 0)

	store.ForEachDoc(func(id int, doc []byte) bool {
		c := &cluster.Cluster{}
		err := json.Unmarshal(doc, c)
		if err != nil {
			return false
		}
		c.Hosts = []*cluster.Host{}
		c.Components = []*cluster.Component{}
		cs = append(cs, c)
